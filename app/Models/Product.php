...
class Product extends Model
{
    ...
    protected static function booted()
    {
        ...
        static::deleting(function ($product) {
            try {
                ...
                // Delete all color images before deleting colors using WebPImageService
                ...
                foreach ($colors as $color) {
                    ...
                    if ($rawImagePath) {
                        try {
                            ...
                            Log::info("Deleted color image using WebPImageService", [
                                'color_id' => $color->id,
                                'image_path' => $rawImagePath
                            ]);
                        } catch (\Exception $e) {
                            Log::warning("Failed to delete color image: " . $e->getMessage(), [
                                'color_id' => $color->id,
                                'image_path' => $rawImagePath
                            ]);
                        }
                ...
                // Delete main product image using WebPImageService
                $rawMainImagePath = $product->getRawImagePath();
                if ($rawMainImagePath) {
                    try {
                        $webpService->deleteImage($rawMainImagePath);
                        Log::info("Deleted main product image using WebPImageService", [
                            'product_id' => $product->id,
                            'image_path' => $rawMainImagePath
                        ]);
                    } catch (\Exception $e) {
                        Log::warning("Failed to delete main product image: " . $e->getMessage(), [
                            'product_id' => $product->id,
                            'image_path' => $rawMainImagePath
                        ]);
                    }
    ...
    public function scopeFilterByActiveDeals($query)
    {
        ...
        $hasProductIds = \Illuminate\Support\Facades\Schema::hasColumn('deals', 'product_ids');
        ...
        if ($hasCategoryIds) {
            $clauses[] = "(deals.applies_to = 'categories' AND JSON_SEARCH(deals.category_ids, 'one', products.category_id) IS NOT NULL)";
        }
        if ($hasProductIds) {
            $clauses[] = "(deals.applies_to IN ('products', 'products_and_services') AND JSON_SEARCH(deals.product_ids, 'one', products.id) IS NOT NULL)";
        }
