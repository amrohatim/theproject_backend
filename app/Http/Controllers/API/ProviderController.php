...
class ProviderController extends Controller
{
    ...
    public function getProductsByCategory($categoryId, Request $request)
    {
        try {
            ...
            Log::info("Getting products for category ID: {$categoryId}, include subcategories: " . ($includeSubcategories ? 'true' : 'false'));
    ...
    public function getAnalytics()
    {
        try {
            ...
            $topProviders = Provider::select('providers.*', DB::raw('COUNT(provider_products.id) as product_count'))
                ->leftJoin('provider_profiles', 'providers.id', '=', 'provider_profiles.provider_id')
                ->leftJoin('provider_products', 'provider_profiles.id', '=', 'provider_products.provider_id')
    ...
    public function getProviderLocations($providerId)
    {
        try {
            ...
            $providerProfile = ProviderProfile::where('provider_id', $providerId)->first();

            if (!$providerProfile) {
                Log::warning("Provider profile not found for provider ID: {$providerId}");
                return response()->json([
                    'success' => true,
                    'locations' => [],
                ]);
            ...
            Log::info("Fetched {$locations->count()} locations for provider ID: {$providerId}, provider profile ID: {$providerProfile->id}");
