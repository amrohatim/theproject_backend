...
class SearchController extends Controller
{
    ...
    private function filterServices(Request $request)
    {
        ...
        // Apply duration filter (hours) - only if both min and max are provided and valid
        if ($request->filled('min_hours') && $request->filled('max_hours')) {
            $minHours = (int) $request->min_hours;
            $maxHours = (int) $request->max_hours;

            // Validate duration range
            if ($minHours >= 0 && $maxHours >= 0 && $maxHours >= $minHours) {
                $query->filterByDuration($minHours, $maxHours);
            }
        }
