...
class CategoryController extends Controller
{
    ...
    public function categoriesWithDeals(Request $request)
    {
        ...
        foreach ($activeDeals as $deal) {
            if ($deal->applies_to === 'all') {
                // For deals that apply to all products, get all categories that have products
                $allCategoriesWithProducts = Category::whereHas('products')->pluck('id');
                $categoryIdsWithDeals = $categoryIdsWithDeals->merge($allCategoriesWithProducts);
            } elseif ($deal->applies_to === 'categories' && !empty($deal->category_ids)) {
                // For category-specific deals
                $dealCategoryIds = is_string($deal->category_ids)
                    ? json_decode($deal->category_ids, true)
                    : $deal->category_ids;

                if (is_array($dealCategoryIds)) {
                    $categoryIdsWithDeals = $categoryIdsWithDeals->merge($dealCategoryIds);
                }
    ...
    private function categoryHasProductsWithDeals($category, $activeDeals)
    {
        ...
        foreach ($activeDeals as $deal) {
            if ($deal->applies_to === 'all') {
                return true; // Deal applies to all products
            }
            ...
            if ($deal->applies_to === 'categories') {
                ...
                if (is_array($dealCategoryIds) && in_array($category->id, $dealCategoryIds)) {
                    return true;
                }
            }
