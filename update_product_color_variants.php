...
try {
    ...
    foreach ($products as $product) {
        ...
        // Add new color variants for remaining available colors
        foreach ($availableColors as $colorKey => $colorInfo) {
            $colorData = $colorInfo['data'];
            echo "      ➕ Adding new color variant: {$colorData['name']}\n";
            ...
            if (copy($sourceImagePath, $fullStorageImagePath)) {
                ...
                $maxOrder = $product->colors()->max('display_order') ?? -1;
                ...
                echo "         ✅ Created new color variant: {$colorData['name']} ({$colorData['hex']})\n";
                $updatedCount++;
                $matchedImages++;
            } else {
                $error = "Failed to copy image: {$colorInfo['filename']}";
