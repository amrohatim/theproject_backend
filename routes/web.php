...
// Image test routes
Route::get('/image-test', [App\Http\Controllers\ImageTestController::class, 'index'])->name('test.images');
Route::post('/image-test/upload', [App\Http\Controllers\ImageTestController::class, 'upload'])->name('test.upload');
Route::get('/api-image-test', [App\Http\Controllers\ImageTestController::class, 'apiTest'])->name('test.api.images');
Route::get('/flutter-image-test', function() {
    return view('flutter_image_test');
})->name('flutter.image.test');
Route::get('/admin/image-test', function() {
    return view('admin.image_test');
})->name('admin.image.test');
Route::get('/admin/fix-images', [\App\Http\Controllers\Admin\ImageFixController::class, 'fixImages'])->name('admin.fix.images');

// Service image test route
Route::get('/test/service-images', function() {
    $services = \App\Models\Service::whereNotNull('image')->with(['branch', 'category'])->take(5)->get();
    return view('test.service_images', compact('services'));
})->name('test.service.images');

// Test vendor services page without authentication
Route::get('/test/vendor-services', function() {
    // Get services for a specific vendor (Mike Vendor who has the service with image)
    $services = \App\Models\Service::whereHas('branch.company', function($query) {
        $query->whereHas('user', function($userQuery) {
            $userQuery->where('email', 'mike@vendor.com');
        });
    })->with(['branch', 'category'])->paginate(10);

    return view('vendor.services.index', compact('services'));
})->name('test.vendor.services');

// Test vendor dashboard without authentication
Route::get('/test/vendor-dashboard', function() {
    // Get data for Mike Vendor who has the service with image
    $user = \App\Models\User::where('email', 'mike@vendor.com')->first();
    $company = $user->company;

    $totalBranches = $company->branches()->count();
    $totalProducts = \App\Models\Product::whereHas('branch.company', function($query) use ($user) {
        $query->where('user_id', $user->id);
    })->count();
    $totalServices = \App\Models\Service::whereHas('branch.company', function($query) use ($user) {
        $query->where('user_id', $user->id);
    })->count();
    $totalOrders = 0; // Mock data
    $totalBookings = 0; // Mock data

    $recentProducts = \App\Models\Product::whereHas('branch.company', function($query) use ($user) {
        $query->where('user_id', $user->id);
    })->with('branch')->latest()->take(5)->get();

    $recentServices = \App\Models\Service::whereHas('branch.company', function($query) use ($user) {
        $query->where('user_id', $user->id);
    })->with('branch')->latest()->take(5)->get();
...
// Test vendor product creation page without authentication
Route::get('/test/vendor-product-create', function() {
    ...
    // Get branches for testing (use Mike Vendor's branches)
    $user = \App\Models\User::where('email', 'mike@vendor.com')->first();
    $branches = \App\Models\Branch::whereHas('company', function ($query) use ($user) {
        $query->where('user_id', $user->id);
    })->orderBy('name')->get();

    return view('vendor.products.create', compact('parentCategories', 'branches'));
})->name('test.vendor.product.create');

// Demo routes for viewing dashboards without authentication (for development only)
Route::get('/admin/demo', function () {
    // Create mock data for the admin dashboard
    $totalUsers = 125;
    $totalVendors = 42;
    $totalCustomers = 83;
    $totalProducts = 256;
    ...
    $recentProducts = collect([
        (object)[
            ...
            'is_available' => true,
            'image' => 'https://images.unsplash.com/photo-1556227834-09f1de7a7d14?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1974&q=80',
            'branch' => (object)[
                'name' => 'Beauty Store Main Branch'
            ]
        ],
        (object)[
        ...
        (object)[
            ...
            'original_price' => null,
            'is_available' => false,
            'image' => 'https://images.unsplash.com/photo-1556228578-8c89e6adf883?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1974&q=80',
            'branch' => (object)[
                'name' => 'Beauty Store Main Branch'
            ]
        ]
...
Route::get('/vendor/demo', function () {
    // Create mock data for the vendor dashboard
    $totalBranches = 3;
    $totalProducts = 15;
    $totalServices = 8;
    $totalOrders = 42;
...
Route::get('/provider/demo', function () {
    // Create mock data for the provider dashboard
    $totalProducts = 12;
    $totalOrders = 25;
    ...
    $recentProducts = collect([
        (object)[
            ...
            'is_available' => true,
            'image' => 'https://images.unsplash.com/photo-1566385101042-1a0aa0c1268c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1974&q=80',
            'category' => (object)[
                'name' => 'Vegetables'
            ]
        ],
        ...
        (object)[
            ...
            'original_price' => null,
            'is_available' => true,
            'image' => 'https://images.unsplash.com/photo-1549931319-a545dcf3bc7b?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1974&q=80',
            'category' => (object)[
                'name' => 'Bakery'
            ]
        ]
