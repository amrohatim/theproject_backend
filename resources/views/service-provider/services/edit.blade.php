            ...
            <div class="grid grid-cols-1 md:grid-cols-1 gap-6 {{ app()->getLocale() == 'ar' ? 'md:pl-[35rem]' : 'md:pr-[35rem]' }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_available" name="is_available" type="checkbox" class="focus:ring-[var(--primary)] h-4 w-4 text-[var(--primary)] border-gray-300 dark:border-gray-600 rounded" {{ old('is_available', $service->is_available) ? 'checked' : '' }}>
                            </div>
                    ...
                    <div>
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="home_service" name="home_service" type="checkbox" class="focus:ring-[var(--primary)] h-4 w-4 text-[var(--primary)] border-gray-300 dark:border-gray-600 rounded" value="1" {{ old('home_service', $service->home_service ?? false) ? 'checked' : '' }}>
                            </div>
                            <div class="ml-3 text-sm">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('service_provider.current_additional_images') }}</label>
                        <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach($existingAdditionalImages as $serviceImage)
                                <div class="border border-gray-200 dark:border-gray-700 rounded-md p-2">
                                    <img src="{{ $serviceImage->image_path }}" alt="{{ __('service_provider.additional_images') }}" class="h-24 w-full object-cover rounded-md">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('service_provider.additional_images_optional_up_to', ['count' => 8]) }}</label>
                        @if($remainingAdditionalSlots > 0)
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('service_provider.additional_images_remaining', ['count' => $remainingAdditionalSlots]) }}</p>
                            <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                @for ($i = 0; $i < $remainingAdditionalSlots; $i++)
                                    <div class="relative flex items-center justify-center px-4 py-4 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md h-28 overflow-hidden" data-preview-container>
                                        <img id="additional-image-preview-{{ $i }}" class="hidden absolute inset-2 h-[calc(100%-1rem)] w-[calc(100%-1rem)] object-cover rounded-md" alt="{{ __('service_provider.additional_images') }} {{ $i + 1 }}">
                                        <label class="absolute inset-0 flex items-center justify-center text-sm text-gray-600 dark:text-gray-300 cursor-pointer transition" data-upload-label>
                                            <span class="block" data-upload-label-text>{{ __('service_provider.upload_file') }}</span>
                        ...
                        @else
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('service_provider.additional_images_max_reached', ['count' => 8]) }}</p>
                        @endif
