            ...
            <div id="basic-panel" class="mb-8 bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                            ...
                            @foreach($parentCategories ?? [] as $parentCategory)
                                <optgroup label="{{ $parentCategory->name }}">
                                    ...
                                    @foreach($parentCategory->children as $childCategory)
                                        <option value="{{ $childCategory->id }}" data-category-id="{{ $childCategory->id }}" {{ old('category_id', $product->category_id) == $childCategory->id ? 'selected' : '' }}>&nbsp;&nbsp;{{ $childCategory->name }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="branch_id" name="branch_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            ...
                            @foreach($branches ?? [] as $branch)
                                <option value="{{ $branch->id }}" data-business-type="{{ $branch->business_type }}" {{ old('branch_id', $product->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_available" name="is_available" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 dark:border-gray-600 rounded" {{ old('is_available', $product->is_available) ? 'checked' : '' }} value="1">
                            </div>
                    ...
                    <div class="col-span-2">
                        <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                        <textarea id="description" name="description" rows="4" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" placeholder="Product description...">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700 dark:text-blue-300">
                                        Product images are associated with colors. The color marked as default will have its image used as the main product image.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="color-item border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-3">
                                ...
                                <select name="colors[{{ $index }}][name]" class="color-name-select focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" required>
                                    ...
                                    @php
                                        $colorOptions = ['Red', 'Crimson', 'FireBrick', 'DarkRed', 'IndianRed', 'LightCoral', 'Salmon', 'DarkSalmon', 'LightSalmon', 'Orange', 'DarkOrange', 'Coral', 'Tomato', 'Gold', 'Yellow', 'LightYellow', 'LemonChiffon', 'Khaki', 'DarkKhaki', 'Green', 'Lime', 'ForestGreen', 'DarkGreen', 'SeaGreen', 'MediumSeaGreen', 'LightGreen', 'PaleGreen', 'SpringGreen', 'MediumSpringGreen', 'YellowGreen', 'Olive', 'DarkOliveGreen', 'Blue', 'MediumBlue', 'DarkBlue', 'Navy', 'SkyBlue', 'LightSkyBlue', 'DeepSkyBlue', 'DodgerBlue', 'SteelBlue', 'CornflowerBlue', 'RoyalBlue', 'LightBlue', 'PowderBlue', 'Purple', 'MediumPurple', 'BlueViolet', 'Violet', 'Orchid', 'Magenta', 'Fuchsia', 'DeepPink', 'HotPink', 'LightPink', 'PaleVioletRed', 'Brown', 'SaddleBrown', 'Sienna', 'Chocolate', 'Peru', 'Tan', 'RosyBrown', 'SandyBrown', 'BurlyWood', 'Wheat', 'NavajoWhite', 'Black', 'DimGray', 'Gray', 'DarkGray', 'Silver', 'LightGray', 'Gainsboro', 'WhiteSmoke', 'White'];
                                    @endphp
                            ...
                            <div class="col-span-2">
                                ...
                                <input type="text" name="colors[{{ $index }}][color_code]" placeholder="#FF0000" value="{{ old('colors.'.$index.'.color_code', $color->color_code) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price Adjustment</label>
                                <input type="number" step="0.01" name="colors[{{ $index }}][price_adjustment]" placeholder="0.00" value="{{ old('colors.'.$index.'.price_adjustment', $color->price_adjustment ?? 0) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Total Stock</label>
                                <input type="number" name="colors[{{ $index }}][stock]" placeholder="10" value="{{ old('colors.'.$index.'.stock', $color->stock ?? 0) }}" min="0" class="color-stock-input focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md transition-colors duration-200">
                            ...
                            <div class="col-span-2">
                                ...
                                <input type="number" name="colors[{{ $index }}][display_order]" placeholder="0" value="{{ old('colors.'.$index.'.display_order', $color->display_order ?? $index) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                            </div>
                        ...
                        <div class="mt-4">
                            <div class="flex flex-col">
                                ...
                                <!-- Image Preview Container -->
                                <div class="image-preview-container mb-3 {{ $color->image ? 'has-image' : '' }}" style="width: 300px; height: 400px; border: 2px dashed #d1d5db; border-radius: 8px; background-color: #f9fafb; display: flex; align-items: center; justify-content: center; overflow: hidden; position: relative;">
                                    @if($color->image)
                                        <img class="image-preview" src="{{ $color->image }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 6px;" alt="Image Preview">
                                ...
                                <input type="file" name="color_images[{{ $index }}]" class="color-image-input block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-gray-700 dark:file:text-gray-200 mb-2" accept="image/*" style="max-width: 300px;">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="flex items-start">
                                        <input type="checkbox" name="colors[{{ $index }}][is_default]" value="1" class="default-color-checkbox focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded mt-1" {{ $color->is_default ? 'checked' : '' }}>
                ...
                <div id="specifications-container" class="space-y-4">
                    ...
                    <div class="specification-item grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-5">
                            ...
                            <input type="text" name="specifications[{{ $index }}][key]" placeholder="Material" value="{{ old('specifications.'.$index.'.key', $specification->key) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                        </div>
                        <div class="col-span-5">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Value</label>
                            <input type="text" name="specifications[{{ $index }}][value]" placeholder="Cotton" value="{{ old('specifications.'.$index.'.value', $specification->value) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Order</label>
                            <input type="number" name="specifications[{{ $index }}][display_order]" placeholder="0" value="{{ old('specifications.'.$index.'.display_order', $specification->display_order ?? $index) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">
                        </div>
                        <div class="col-span-1 flex items-end justify-center">
                            <button type="button" class="remove-item text-red-500 hover:text-red-700">
                                <i class="fas fa-trash"></i>
