    ...
    <div class="mb-6">
        <div class="overflow-x-auto">
            <nav class="inline-flex min-w-full whitespace-nowrap border-b border-gray-200 dark:border-gray-700">
                <a
                    href="{{ route('vendor.products.index', $tabBaseQuery) }}"
                ...
                @foreach($branches as $branch)
                    <a
                        href="{{ route('vendor.products.index', array_merge($tabBaseQuery, ['branch' => $branch->id])) }}"
        ...
        <div class="block md:hidden space-y-4 px-4 py-4">
            @forelse($products ?? [] as $product)
                <x-mobile-product-card
                    :product="$product"
                    :edit-url="route('vendor.products.edit', $product->id)"
                    :delete-url="route('vendor.products.destroy', $product->id)"
                    :delete-confirm="__('messages.delete_product_confirmation')"
                />
            @empty
                <div class="text-center py-12 px-4">
                    ...
                    <a href="{{ route('vendor.products.create') }}" class="mt-3 inline-flex items-center px-4 py-2 bg-[var(--primary)] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[var(--primary)] active:bg-[var(--primary)] focus:outline-none focus:border-[var(--primary)] focus:ring ring-[var(--primary)] disabled:opacity-25 transition ease-in-out duration-150">
                        <i class="fas fa-plus mr-2"></i> {{ __('messages.add_product') }}
                    </a>
                </div>
            @endforelse
            ...
            <table class="responsive-table min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap" data-label="License Status">
                            @php
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" data-label="{{ __('messages.actions') }}">
                            @if($canEdit)
                                <a href="{{ route('vendor.products.edit', $product->id) }}" class="text-[var(--primary)] dark:text-[var(--primary)] hover:text-[var(--primary)] dark:hover:text-[var(--primary)] mr-3" title="Edit Product">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                            <div class="flex flex-col items-center justify-center py-4">
                                ...
                                <a href="{{ route('vendor.products.create') }}" class="mt-2 inline-flex items-center px-4 py-2 bg-[var(--primary)] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[var(--primary)] active:bg-[var(--primary)] focus:outline-none focus:border-[var(--primary)] focus:ring ring-[var(--primary)] disabled:opacity-25 transition ease-in-out duration-150">
                                    <i class="fas fa-plus mr-2"></i> {{ __('messages.add_product') }}
                                </a>
                            </div>
                        </td>
