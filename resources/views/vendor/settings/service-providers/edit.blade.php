...
@section('title', __('vendor.edit_service_provider'))
@section('page-title', __('vendor.edit_service_provider'))
...
<div class="container mx-auto">
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white">{{ __('vendor.edit_service_provider') }}</h2>
            <p class="mt-1 text-gray-600 dark:text-gray-400">{{ __('vendor.edit_service_provider_description') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <form method="POST" action="{{ route('vendor.settings.service-providers.update', $serviceProvider) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-input-container has-label">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $serviceProvider->user->name) }}" required
                ...
                <div class="form-input-container has-label">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $serviceProvider->user->email) }}" required
                           class="form-input {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}"
                           dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                    @error('email')
                        <span class="field-error-message {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ $message }}</span>
                ...
                <div class="form-input-container has-label">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $serviceProvider->user->phone) }}"
                           class="form-input {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}"
                           dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                    @error('phone')
                        <span class="field-error-message {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ $message }}</span>
                ...
                <!-- Status -->
                <div class="form-input-container has-label">
                    <label for="status" class="form-label {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('vendor.account_status') }} <span class="text-red-500">*</span></label>
                    <select name="status" id="status" required
                            class="form-select {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}"
                            dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                        <option value="active" {{ old('status', $serviceProvider->user->status) === 'active' ? 'selected' : '' }}>{{ __('vendor.active') }}</option>
                        <option value="inactive" {{ old('status', $serviceProvider->user->status) === 'inactive' ? 'selected' : '' }}>{{ __('vendor.inactive') }}</option>
                    </select>
                    @error('status')
                        <span class="field-error-message {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ $message }}</span>
                    @enderror
                ...
                <!-- Password -->
                <div class="form-input-container has-label">
                    <label for="password" class="form-label {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('vendor.new_password') }}</label>
                ...
                <!-- Confirm Password -->
                <div class="form-input-container has-label">
                    <label for="password_confirmation" class="form-label {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('vendor.confirm_password') }}</label>
                            ...
                            @foreach($branches as $branch)
                                <div class="flex items-center gap-2 {{ app()->getLocale() === 'ar' ? 'flex-row-reverse' : '' }}">
                                    <input type="checkbox" name="branch_ids[]" value="{{ $branch->id }}" id="branch_{{ $branch->id }}"
                                           ...
                                           {{ in_array($branch->id, old('branch_ids', $serviceProvider->branch_ids ?? [])) ? 'checked' : '' }}>
                                ...
                                @foreach($services as $service)
                                    <div class="flex items-center {{ app()->getLocale() === 'ar' ? 'flex-row-reverse' : '' }}">
                                        <input type="checkbox" name="service_ids[]" value="{{ $service->id }}" id="service_{{ $service->id }}"
                                               class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                                               {{ in_array($service->id, old('service_ids', $serviceProvider->service_ids ?? [])) ? 'checked' : '' }}>
            ...
            <div class="mt-8 flex {{ app()->getLocale() === 'ar' ? 'justify-start' : 'justify-end' }} space-x-3 {{ app()->getLocale() === 'ar' ? 'rtl:space-x-reverse' : '' }}">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest focus:outline-none disabled:opacity-25 transition ease-in-out duration-150" style="background-color: #53D2DC; border-color: #53D2DC;" onmouseover="this.style.backgroundColor='#42B8C2'" onmouseout="this.style.backgroundColor='#53D2DC'" onfocus="this.style.boxShadow='0 0 0 3px rgba(83, 210, 220, 0.3)'" onblur="this.style.boxShadow='none'">
                    <i class="fas fa-save {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i> {{ __('vendor.update_service_provider_button') }}
                </button>
            </div>
        </form>
    </div>
