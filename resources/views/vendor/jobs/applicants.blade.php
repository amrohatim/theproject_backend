                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-3">
                                @if($application->user_cv)
                                    <a href="{{ route('vendor.jobs.applicants.cv', $application->id) }}" target="_blank" rel="noopener" class="inline-flex items-center px-3 py-1 text-xs font-semibold text-green-700 bg-green-100 rounded-md hover:bg-green-200">
                                        Show CV
                                    </a>
                                @endif
                                <form method="POST" action="{{ route('vendor.jobs.applicants.destroy', $application->id) }}" onsubmit="return confirm('Delete this application?');">
                                    @csrf
                                    @method('DELETE')
