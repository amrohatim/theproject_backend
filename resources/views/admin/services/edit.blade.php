                        ...
                        <select id="category_id" name="category_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            ...
                            @foreach($parentCategories ?? [] as $parentCategory)
                                <optgroup label="{{ $parentCategory->name }}">
                                    ...
                                    <option value="{{ $parentCategory->id }}" {{ old('category_id', $service->category_id) == $parentCategory->id ? 'selected' : '' }}>{{ $parentCategory->name }}</option>
                    ...
                    <div>
                        ...
                        <select id="branch_id" name="branch_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            ...
                            @foreach($branches ?? [] as $branch)
                                <option value="{{ $branch->id }}" {{ old('branch_id', $service->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }} ({{ $branch->company->name ?? 'No Company' }})</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-start mt-4">
                        ...
                        <div class="ml-3 text-sm">
                            ...
                            <p class="text-gray-500 dark:text-gray-400">Mark this service as featured to highlight it on the homepage and in search results.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start mt-4">
                        <div class="flex items-center h-5">
                            <input id="is_available" name="is_available" type="checkbox" value="1" {{ old('is_available', $service->is_available) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded">
                        </div>
                        <div class="ml-3 text-sm">
                            <label for="is_available" class="font-medium text-gray-700 dark:text-gray-300">Available</label>
                            <p class="text-gray-500 dark:text-gray-400">Mark this service as available for booking.</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md">
                            <div class="space-y-1 text-center">
                                @if($service->image)
                                    <img src="{{ asset(str_replace('/storage/', 'storage/', $service->image)) }}" alt="{{ $service->name }}" class="mx-auto h-32 w-auto object-cover mb-4">
                                @else
