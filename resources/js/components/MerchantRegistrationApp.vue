          ...
          <div class="space-y-6 mt-12">
            <div class="flex items-start space-x-4">
              <div class="flex-shrink-0">
                <svg class="w-6 h-6" style="color: rgba(255, 255, 255, 0.85);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                </svg>
              </div>
              <div>
                <h3 class="font-semibold text-lg mb-1">{{ $t('grow_your_sales') }}</h3>
                <p class="text-sm" style="color: rgba(255, 255, 255, 0.9);">{{ $t('access_large_customer_base') }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start space-x-4">
              ...
              <div>
                ...
                <p class="text-sm" style="color: rgba(255, 255, 255, 0.9);">{{ $t('easy_dashboard_manage_products') }}</p>
              </div>
            </div>
            ...
            <div class="flex items-start space-x-4">
              ...
              <div>
                ...
                <p class="text-sm" style="color: rgba(255, 255, 255, 0.9);">{{ $t('our_team_help_succeed') }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="w-full lg:w-1/2 bg-white p-8 lg:p-12 flex items-center justify-center">
        <div class="w-full max-w-md space-y-6">
          ...
          <div class="w-full mb-8">
              ...
              <div class="flex items-center">
                <div class="flex flex-col items-center group">
                  ...
                  <div class="mt-3 text-center">
                    <div class="text-xs font-semibold" :class="getStepLabelClasses(1)" :style="getStepLabelStyles(1)">{{ translatedSteps[0].name }}</div>
                  </div>
                </div>
                <div class="progress-line flex-1 h-1 mx-4 rounded-full"
                     :style="{ backgroundColor: currentStep > 1 ? 'var(--primary)' : '#d1d5db' }"></div>
              </div>
              <div class="flex items-center">
                <div class="flex flex-col items-center group">
                  <div class="step-circle w-12 h-12 rounded-full flex items-center justify-center text-sm font-semibold border-2"
                       :class="getStepClasses(2)"
                       :style="getStepStyles(2)"
                       data-step="2"
                       tabindex="0"
                       role="button"
                  ...
                  <div class="mt-3 text-center">
                    <div class="text-xs font-semibold" :class="getStepLabelClasses(2)" :style="getStepLabelStyles(2)">{{ translatedSteps[1].name }}</div>
                  </div>
                </div>
                <div class="progress-line flex-1 h-1 mx-4 rounded-full"
                     :style="{ backgroundColor: currentStep > 2 ? 'var(--primary)' : '#d1d5db' }"></div>
              </div>
              <div class="flex items-center">
                <div class="flex flex-col items-center group">
                  <div class="step-circle w-12 h-12 rounded-full flex items-center justify-center text-sm font-semibold border-2"
                       :class="getStepClasses(3)"
                       :style="getStepStyles(3)"
                       data-step="3"
                       tabindex="0"
                       role="button"
                  ...
                  <div class="mt-3 text-center">
                    <div class="text-xs font-semibold" :class="getStepLabelClasses(3)" :style="getStepLabelStyles(3)">{{ translatedSteps[2].name }}</div>
                  </div>
                </div>
                <div class="progress-line flex-1 h-1 mx-4 rounded-full"
                     :style="{ backgroundColor: currentStep > 3 ? 'var(--primary)' : '#d1d5db' }"></div>
              </div>
              <div class="flex items-center">
                <div class="flex flex-col items-center group">
                  <div class="step-circle w-12 h-12 rounded-full flex items-center justify-center text-sm font-semibold border-2"
                       :class="getStepClasses(4)"
                       :style="getStepStyles(4)"
                       data-step="4"
                       tabindex="0"
                       role="button"
                  ...
                  <div class="mt-3 text-center">
                    <div class="text-xs font-semibold" :class="getStepLabelClasses(4)" :style="getStepLabelStyles(4)">{{ translatedSteps[3].name }}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="md:hidden">
              <div class="flex items-center justify-between mb-4">
                <span class="text-sm font-semibold" id="mobile-step" style="color: var(--primary);">{{ $t('step_of').replace('{current}', currentStep).replace('{total}', steps.length) }}</span>
              ...
              <div class="w-full bg-gray-200 rounded-full h-3 mb-4 shadow-inner">
                <div class="h-3 rounded-full transition-all duration-500 ease-out shadow-sm"
                     :style="`width: ${(currentStep / translatedSteps.length) * 100}%; background: linear-gradient(135deg, var(--primary) 0%, var(--primary-hover) 100%);`"></div>
              </div>
              ...
              <div class="flex justify-center mt-4 space-x-2" id="mobile-dots">
                <div v-for="(step, index) in translatedSteps" :key="index"
                     ...
                     :style="{ backgroundColor: currentStep === index + 1 ? 'var(--primary)' : '#d1d5db' }"
          ...
          <div v-if="currentStep === 5" class="text-center">
            ...
            <button @click="goToLogin" class="px-6 py-2 text-white rounded-md"
          ...
          <div class="text-center">
            <p class="text-sm text-gray-600">
              ...
              <a href="/login" class="font-medium"
                 ...
                 onmouseout="this.style.color='var(--primary)'">{{ $t('log_in') }}</a>
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getStepLabelStyles(stepNumber) {
      const isActive = stepNumber <= this.currentStep;
      return isActive ? { color: 'var(--primary)' } : {};
    },
